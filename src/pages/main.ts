...
for (let i: number = 0; i < brickColumnCount; i++) {
  ...
  for (let j: number = 0; j < brickRowCount; j++) {
    const brickX: number = i * (brickWidth + brickPadding) + brickOffsetLeft;
    const brickY: number = j * (brickHeight + brickPadding) + brickOffsetTop;
...
function ballMovement(): void {
  ...
  const isBallSameXAsPaddle: boolean = x > paddleX && x < paddleX + paddleWidth;
